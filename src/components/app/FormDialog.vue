<template>
  <v-dialog v-model="dialog" :max-width="maxWidth">
    <template v-if="enableAddButton" v-slot:activator="{ on, attrs }">
      <v-btn
        color="primary"
        fab
        dark
    ...
    <v-card>
      ...
      <v-card-text :class="{ 'no-padding': noPadding }" class="pt-4">
        <v-container :class="{ 'no-padding': noPadding }">
          <v-form @submit.prevent="accept" ref="form" :disabled="disabled">
            <slot />
          </v-form>
        </v-container>
      </v-card-text>
